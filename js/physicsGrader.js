'use strict';
// PhysicsGrader — AP Physics 1 FRQ Auto-Grader
// Implements four grading protocols:
//   1. Physics Bucket Scan  — keyword detection with fuzzy matching
//   2. 15-word Proximity Check — cause + connector + effect within window
//   3. QQT Rule             — derivation items must cite a Fundamental Principle
//   4. CPE Protection       — consistent carry-forward error credited in later parts
//
// Public API:  window.PhysicsGrader.grade(answerObj, frqType, unitNum, question) → gradeResult

(function () {

  // ── Fundamental Principles list (for QQT check) ─────────────────────────────
  var FUNDAMENTAL_PRINCIPLES = [
    "newton's second law", "newton's 2nd law",
    "f_net = ma", "f = ma", "fnet = ma",
    "net force equals mass times acceleration",
    "sum of forces", "∑f = ma", "sigma f",
    "second law of motion",
    "conservation of energy",
    "work-energy theorem", "work energy theorem",
    "w_net = delta ke", "wnet = delta ke",
    ...
    "free fall", "y = 1/2 g t", "h = 1/2 g t",
    "v = v0 + at", "x = v0t + 1/2 at"
  ];
  ...
  // ── Protocol 1: Physics Bucket Scan ─────────────────────────────────────────
  ...
  function qqtCheck(answerText) {
    var lower = answerText.toLowerCase();
    for (var i = 0; i < FUNDAMENTAL_PRINCIPLES.length; i++) {
      if (lower.indexOf(FUNDAMENTAL_PRINCIPLES[i]) !== -1) return true;
    }
    return false;
  }

  // ── Protocol 4: CPE Protection ───────────────────────────────────────────────
  // Returns true if the student's earlier source part was wrong but this part
  // uses a value that appears consistent with what they computed in the source part.
  function cpeCheck(answerObj, rubricItem, gradeLog) {
    ...
    var thisLabel  = rubricItem.skill === 'calculation' ? 'c' : sourceLabel;
    // Try to find which answer text to check; use full answer obj values if label uncertain
    var thisText = '';
    ...
    // Extract numbers from source part answer; see if any appear in this text
  ...
  function grade(answerObj, frqType, unitNum, question) {
    ...
    rubric.forEach(function(item) {
      ...
      // ── Protocol 3: QQT check ─────────────────────────────────────────────
      if (item.fundamentalPrinciple && awarded && !qqtCheck(text)) {
        awarded = false;
        reason  = 'QQT: Fundamental Principle not cited (e.g. Newton\u2019s 2nd Law, Work-Energy Theorem)';
      ...
      // Log result keyed by part skill for CPE look-up by subsequent items
  ...
  // ── Public API ──────────────────────────────────────────────────────────────
  window.PhysicsGrader = { grade: grade };
